...
for lease in report_dataset:
        print('| <a href="http://' + format(lease['ip_address'], '<15') + '" target="_blank">' + format(lease['ip_address'], '<15') + '</a> | ' + \
                format(lease['hardware'], '<17') + ' | ' + \
                format(str((lease['ends'] - now) if lease['ends'] != 'never' else 'never'), '>20') + ' | ' + \
                format(lease['client-hostname'], '<24') + '|')

print('+-----------------+-------------------+----------------------+-------------------------+')
print('| Total Active Leases: ' + format(str(len(report_dataset)), '<64') + '|' )
print('| Report generated (UTC): ' + format(str(now), '<61') + '|' )
print('+--------------------------------------------------------------------------------------+')
